...
def _get_lama(**kwargs):
    return [
        "--if-unit-cost",
        "--evaluator",
        "hlm=lmcount(lm_acyclic(lm_rhw(reasonable_orders=true)),pref={pref})".format(**kwargs),
        "--evaluator", "hff=ff()",
        "--search", """iterated([
        ...
        "--if-non-unit-cost",
        "--evaluator",
        "hlm1=lmcount(lm_acyclic(lm_rhw(reasonable_orders=true)),transform=adapt_costs(one),pref={pref})".format(**kwargs),
        "--evaluator", "hff1=ff(transform=adapt_costs(one))",
        "--evaluator",
        "hlm2=lmcount(lm_acyclic(lm_rhw(reasonable_orders=true)),transform=adapt_costs(plusone),pref={pref})".format(**kwargs),
...
ALIASES["lama-first"] = [
    "--evaluator",
    "hlm=lmcount(lm_acyclic(lm_rhw(reasonable_orders=true)),transform=adapt_costs(one),pref=false)",
    "--evaluator", "hff=ff(transform=adapt_costs(one))",
